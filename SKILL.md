...
| `acp wallet balance --ticker <ticker> --json` | **Balance of ONE thing — use this for "how much of X do I have?" and pre-trade sizing; prefer it over the full-portfolio forms below when you only care about one ticker.** `--ticker` (alias: `--token`) takes a ticker (e.g. `VIRTUAL`) or a contract/mint address; an address needs no chain and is the surest input, since it can't be confused with a same-ticker token. **Pass no chain flag** — the backend resolves the ticker against the wallet (trade history, then holdings across every chain plus Hyperliquid). `--chain-id <id>` narrows the search and is only right when the user named a chain. | `{symbol, total, registryOnly, filteredUnpriced, balances:[{chainId, network, tokenAddress, symbol, walletAddress, raw, decimals, balance}], networksChecked:[…], unresolvedNetworks:[…], stocks:{positions:[…], unavailable}, hyperliquid:{spotBalances:[…], positions:[…]}\|null}`. `total` is the summed **on-chain** holding (already decimal-shifted; don't divide again) — tokenized stocks and Hyperliquid are reported separately and NOT added into it, since shares and perp size aren't token balances. **`total` is null, not `"0"`, when no balance was read** (`registryOnly:true` means the ticker resolved but nothing says the owner holds any) — never render null as zero. `filteredUnpriced > 0` means rows were dropped for having no market: a `"0"` total then means "none of the real token", not "nothing at all". A network in `unresolvedNetworks`, or `stocks.unavailable:true`, means that part wasn't read — never report a zero as "holds none" then. |
| `acp wallet balance [--chain-id <id>] [--cluster <c>] --json` | Token balances (full portfolio; for a single token prefer `--ticker` above). No flags → all sponsored EVM chains + Solana for the env. `--chain-id` narrows to one chain (EVM, or `500`/`501` for Solana); `--cluster devnet\|mainnet` → Solana only | Single chain (`--chain-id`/`--cluster`): `{chainId, network, address, tokens:[…]}`. All-chains (no flags): `{chains:[{chainId, network}], address, solanaAddress, tokens:[…]}` — `tokens` each carry `network`; group by it. Each token: `{tokenAddress, tokenBalance, tokenMetadata:{symbol, name, decimals}, tokenPrices:[{value}]}` (`tokenBalance` raw; decimal-shift by `decimals`, native token has `tokenAddress:null`). The all-chains view also carries `stocks:[{ticker, token_ticker, chain, protocol, shares, usd_value, ...}]` — your tokenized-stock holdings resolved per venue (`chain` is the value a stock sell's `--chain` needs — `eth`, `base`, `sol`, or `robinhood`; `shares` is the sellable amount) — and a `hyperliquid` account summary |
...
> **One command does the whole route — never chain trades yourself.** A single `acp trade` is decomposed by the backend into however many legs the route needs and the CLI signs each in sequence, blocking until the **last** leg settles. Asking for `USDC@HL → ETH@Base` runs multiple legs in one call (withdraw to L1 → bridge+swap to ETH on Base); `VIRTUAL@Base → AAPL` bridges then buys. **Express the trade by its start and end (`--token-in`/`--chain-in` → `--token-out`/`--chain-out`) and let one command plan the path** — do NOT issue a `withdraw` then a separate `swap`, or a `bridge` then a `swap`. (The settle wait is per the slowest leg; a multi-leg route can run minutes — keep the process alive, don't re-issue on a perceived hang.)

**Tokenized stocks (spot) — buy/sell real shares, not a perp.** `acp trade --token <TICKER> --amount-usdc <usd>` buys tokenized equity (you receive the share token); `--amount-shares <n>` sells. This is **spot** (no leverage, no funding, you own the asset) — distinct from an HL equity *perp*. **There are FOUR venues, named by `--chain`:** `eth` (Ethereum 1, settles USDC, protocols `ondo`/`xstocks`), `base` (Base 8453, USDC, protocol `coinbase`), `sol` (Solana, USDC, `ondo`/`xstocks`), and `robinhood` (Robinhood Chain 4663, settles **USDG**, protocol `robinhood`). Not every ticker lists on every venue — `acp trade stock-list <SYMBOL>` shows which ones carry it. On a BUY `--chain` is OPTIONAL: the backend quotes `eth`/`base`/`sol` and takes the best fill the wallet can fund in place; pass it to pin one. **`robinhood` is never auto-picked** — reach it only with an explicit `--chain robinhood`. Fund a buy with the venue's dollar you already hold, or (USDC venues only) from another chain via `--token-in`/`--chain-in` — it bridges first. **Sells REQUIRE `--chain eth|base|sol|robinhood`** (the server can't see which chain holds your shares — omitting it fails with `VALIDATION_ERROR` before anything executes). **To find the right `--chain` value, run `acp wallet balance --json` and read the `stocks[]` array**: each entry maps the bare ticker to the actual holding — `{ticker: "AAPL", token_ticker: "AAPLx", chain: "sol", shares: "0.0153...", ...}` — its `chain` field is exactly the value to pass as `--chain`, and `shares` is the maximum you can pass as `--amount-shares`. (Don't grep the raw `tokens` list for the bare ticker — the share token's on-chain symbol differs, e.g. `AAPLx`; the `stocks[]` array does the mapping for you.) **A sell settles on the VENUE'S OWN chain** — USDC on Ethereum, Base, or Solana; USDG on Robinhood Chain. **To receive the proceeds as another token and/or on another chain, add `--token-out <token>` and `--chain-out <chainId>`** — the backend sells then bridges onward in one command (e.g. USDC on Base, or ETH on Arbitrum), and this works from the `eth`, `base`, and `sol` venues alike. A non-USDC `--token-out` requires `--chain-out` (the destination chain is never guessed). **`robinhood` is the exception:** no bridge covers chain 4663, so a robinhood buy cannot be funded from another chain and a robinhood sell rejects `--token-out`/`--chain-out` with `VALIDATION_ERROR` — the USDG stays on Robinhood Chain.
...
**Perp markets aren't just crypto.** Hyperliquid lists leveraged perps across multiple asset classes — crypto, **equities/stocks**, **FX/currencies**, and **commodities** — so `acp trade --side long|short --token <SYMBOL>` can open a leveraged position on any of them. Pass the Hyperliquid market symbol as `--token` (e.g. `BTC`, `ETH`, plus the equity/FX/commodity markets HL lists); use `acp trade hl-status` to see your HL account (perp positions and margin). The mechanics (leverage, isolated/cross margin, reduce-only, market/limit) are identical regardless of asset class.

**Discovery — `acp trade stock-list [symbol]` (read-only).** To find out *what* is tradable before constructing a trade. With **no symbol** it returns the tokenized-stock catalog under `stocks` (`symbol`, `name`, `protocols`) — note `protocols` reports only `ondo`/`xstocks` and the catalog deliberately keeps the venue/chain internal, so NEVER read it as the venue list (a ticker can also trade on `base` or `robinhood` without appearing to); a `warnings` field appears only if a venue's catalog is momentarily unavailable. With a **symbol** it returns `{ symbol, name?, routes }`, each route `{ kind, label, token, maxLeverage? }` — **`token` is the exact ticker to pass** (an HL equity perp must be quoted `xyz:AAPL`; the tokenized-stock route uses bare `AAPL`). Use it to resolve the right ticker, then build the trade with the flags above. **It never implies you must pre-hold USDC on Hyperliquid: USDC is the settlement currency, not a prerequisite — any trade can be funded with any supported token on any supported chain via `--token-in`/`--chain-in`, and the backend bridges/swaps/deposits to settle.**

Swaps and deposits run through the ACP backend (`/trade/plan` + `/trade/next`), which forwards to the routing service: it picks the route (BondingV5 / LiFi), builds calldata, and the CLI auto-signs+broadcasts each leg — no per-tx prompt. HL perp/withdraw are EIP-712 actions signed by the same keystore signer. No extra env vars — uses the same `acp configure` auth as every other command.

**Auto-balancing (no manual transfer needed).** HL keeps USDC in separate perp and spot wallets, and perp orders draw on the perp wallet. The CLI handles this automatically: before a perp order it tops up the perp wallet from the spot side if short, via an instant free L1 transfer. So a typical flow is just `deposit → perp order` — the funds move themselves. (HL still enforces a ~$10 minimum order value.)

```bash
# Discover what's tradable (read-only). No symbol → the tokenized-stock catalog.
...
# Tokenized stock: BUY $5 of AAPL with USDC you hold (auto-routes across eth/base/sol)
acp trade --token AAPL --amount-usdc 5 --json
# Tokenized stock: BUY pinned to one venue (skips the cross-venue comparison)
acp trade --token AAPL --amount-usdc 5 --chain base --json
# Tokenized stock: BUY on Robinhood Chain — explicit only, spends USDG held on 4663
acp trade --token AAPL --amount-usdc 5 --chain robinhood --json
...
# Tokenized stock: SELL — first find which chain holds the shares (and how many):
acp wallet balance --json   # → stocks:[{ticker:"AAPL", token_ticker:"AAPLx", chain:"sol", shares:"0.0153", ...}]
# ...then sell with that chain (--chain eth|base|sol|robinhood is REQUIRED on sells)
acp trade --token AAPL --amount-shares 0.01 --chain sol --json
# Tokenized stock: SELL 3 AAPL, receive proceeds as USDC on Base (bridges onward)
acp trade --token AAPL --amount-shares 3 --chain eth --token-out usdc --chain-out 8453 --json
# ...or as ETH on Arbitrum — a non-USDC --token-out requires --chain-out (chain never guessed)
acp trade --token AAPL --amount-shares 3 --chain eth --token-out eth --chain-out 42161 --json
...
# Multi-leg in ONE call: exit Hyperliquid to ETH on Base
# (withdraw → bridge+swap; one command, blocks until the last leg)
acp trade --token-in usdc --chain-in 1337 --amount-in 100 --token-out eth --chain-out 8453 --json
...
# HL ACCOUNT status (read-only) — HL perp positions and margin ONLY.
...
| `trade` (tokenized stock) | Spot buy/sell of a tokenized equity (you own the share token; NO `--side`). Backend auto-routes across the `eth`/`base`/`sol` venues; `robinhood` (USDG) is explicit-only | `--token <TICKER>`, and `--amount-usdc` (buy) **or** `--amount-shares` + `--chain eth\|base\|sol\|robinhood` (sell — get the chain from `stocks[]` in `acp wallet balance --json`) | buy can fund from another chain via `--token-in`/`--chain-in`/`--amount-in` (not on `robinhood`); sell can redirect proceeds via `--token-out`/`--chain-out` from the eth/base/sol venues (USDC to any chain, or a non-USDC token when `--chain-out` is set; rejected on `robinhood`); `--chain` (optional on buys to pin the venue, required on sells), `--protocol ondo\|xstocks\|coinbase\|robinhood`, `--slippage`, `--dry-run` |
| `trade stock-list [symbol]` | **Read-only discovery.** No symbol → the tokenized-stock catalog (`stocks`). With a symbol → routes for that asset, each naming the exact `token` to pass | — | `[symbol]` |
| `trade hl-status` | **HL account ONLY**: HL perp positions and margin. For on-chain token balances use `acp wallet balance` | — | — |
...
| `acp agent quote-tokens [--chain-id <id>] --json` | List the assets an Occupy curve can be priced against — `{chainId, quoteTokens:[{symbol, name, address, decimals}]}`. **Call this before an Occupy launch** so you can offer the human real choices (and their decimals) rather than guessing an address. |
| `acp agent tokenize --launchpad occupy --quote-token <symbol\|addr> [--name <name> --pool-fee --tax-bips --no-thicken-liquidity --anti-sniper <0\|1> --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed** (gas is sponsored), curve quoted in `--quote-token`, which is **required and has no default** and takes a **symbol** (`NVDAc`, `TSLAc`, `MSFTc` — see `acp agent quote-tokens`) or an address. It is a **tokenized equity**, not VIRTUAL or a stablecoin, so it decides which stock the agent token trades against. Surface that choice to the human like any other launch economics. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. `--name` sets the token name (Occupy names the token independently of the agent; Virtuals does not, and `--name` is rejected there), and `--anti-sniper` on Occupy offers only `0` (none) or `1` (60s) — `2` is Virtuals-only. |
