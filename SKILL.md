...
| `acp wallet balance --ticker <ticker> --json` | **Balance of ONE thing — use this for "how much of X do I have?" and pre-trade sizing; prefer it over the full-portfolio forms below when you only care about one ticker.** `--ticker` (alias: `--token`) takes a ticker (e.g. `VIRTUAL`) or a contract/mint address; an address needs no chain and is the surest input, since it can't be confused with a same-ticker token. **Pass no chain flag** — it scans the wallet across every chain at once. `--chain-id <id>` narrows the scan to one chain and is only right when the user named one. | `{symbol, total, balances:[{chainId, network, tokenAddress, symbol, walletAddress, raw, decimals, balance}], networksChecked:[…], unresolvedNetworks:[…], stocks:{positions:[…], unavailable}, hyperliquid:{spotBalances:[…], positions:[…]}\|null}` — same shape with or without `--chain-id`. `total` is the summed **on-chain** holding (already decimal-shifted; don't divide again); tokenized stocks and Hyperliquid are reported in their own fields and deliberately NOT added into it, since shares and perp size aren't token balances. A network in `unresolvedNetworks`, or `stocks.unavailable:true`, means that part wasn't read — never report a zero `total` as "holds none" then. `chainId` is null when the backend can't identify the row's network; use `network`. |
