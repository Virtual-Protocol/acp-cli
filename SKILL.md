...
| `trade` (tokenized stock) | Spot buy/sell of a tokenized equity (you own the share token; NO `--side`). Backend auto-routes the venue/chain | `--token <TICKER>`, and `--amount-usdc` (buy) **or** `--amount-shares` (sell) | buy can fund from another chain via `--token-in`/`--chain-in`/`--amount-in`; sell can redirect proceeds via `--token-out`/`--chain-out` (eth venue: USDC to any chain, or a non-USDC token when `--chain-out` is set); `--chain eth\|sol` (required on sells), `--protocol`, `--slippage`, `--dry-run` |
