...
| `acp wallet balance --ticker <ticker> --json` | **Fast single-token balance — use this for "how much of TOKEN do I have?" and pre-trade checks; prefer it over the full-portfolio forms below when you only need one token.** `--ticker` (alias: `--token`) is a ticker (e.g. `VIRTUAL`, resolved to the trade-canonical token) or a contract/mint address. **`--chain-id` is optional and usually unnecessary** — omit it and every sponsored chain plus Solana is checked at once, which is what you want when the user just names a ticker. Add `--chain-id <id>` (EVM id, or `500`/`501` for Solana) only when they named a chain. | Without `--chain-id`: `{symbol, total, balances:[{chainId, network, tokenAddress, symbol, walletAddress, raw, decimals, balance}], chainsChecked:[…], unresolvedChains:[…]}` — `total` is the summed human-readable holding across chains; `balances` says where it sits; a chain in `unresolvedChains` was NOT read, so never report a zero `total` as "holds none" while that list is non-empty. With `--chain-id`: the flat `{chainId, tokenAddress, symbol, walletAddress, raw, decimals, balance}` (`raw` = base units; `balance` = decimal-shifted string) |
