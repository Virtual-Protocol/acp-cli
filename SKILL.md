...
| `acp agent tokenize --launchpad occupy [--name <name> --quote-token <addr> --pool-fee --tax-bips --no-thicken-liquidity --anti-sniper <0–5> --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed** (gas is sponsored), curve quoted in `--quote-token` rather than VIRTUAL. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. `--name` sets the token name (Occupy names the token independently of the agent; Virtuals does not, and `--name` is rejected there), and `--anti-sniper` accepts the extended schedules `3` (98min sell tax), `4` (98min buy+sell tax) and `5` (10min buy tax) on top of `0`–`2`. |
