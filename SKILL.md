...
| `acp agent tokenize --launchpad occupy [--name <name> --quote-token <addr> --pool-fee --tax-bips --no-thicken-liquidity --anti-sniper <0\|1> --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed** (gas is sponsored), curve quoted in `--quote-token` — a **tokenized equity** (NVDAc, AAPLc, TSLAc, …, 8 decimals), not VIRTUAL or a stablecoin. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. `--name` sets the token name (Occupy names the token independently of the agent; Virtuals does not, and `--name` is rejected there), and `--anti-sniper` on Occupy offers only `0` (none) or `1` (60s) — `2` is Virtuals-only. |
