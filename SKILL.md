...
| `acp agent tokenize --launchpad occupy --quote-token <addr> [--name <name> --pool-fee --tax-bips --no-thicken-liquidity --anti-sniper <0\|1> --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed** (gas is sponsored), curve quoted in `--quote-token`, which is **required and has no default** — a **tokenized equity** (NVDAc `0xb20000000000000000000078ee7ce2fE4908108C`, AAPLc, TSLAc, METAc, GOOGLc, MSTRc, AMZNc, SPCXc; 8 decimals), not VIRTUAL or a stablecoin, so it decides which stock the agent token trades against. Surface that choice to the human like any other launch economics. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. `--name` sets the token name (Occupy names the token independently of the agent; Virtuals does not, and `--name` is rejected there), and `--anti-sniper` on Occupy offers only `0` (none) or `1` (60s) — `2` is Virtuals-only. |
