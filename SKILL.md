...
| `acp agent tokenize --launchpad occupy [--quote-token <addr> --pool-fee --tax-bips --no-thicken-liquidity --prebuy]` | Launch on the **Occupy** launchpad instead: one transaction, **no launch fee and no VIRTUAL needed** (gas is sponsored), curve quoted in `--quote-token` rather than VIRTUAL. EVM only. `--acf`/`--60-days`/`--airdrop-percent`/`--robotics` are Virtuals-only and are rejected here. A `--prebuy` is denominated in the quote asset, so it requires `--quote-token`. |
