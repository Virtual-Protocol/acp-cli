...
| `acp wallet balance --ticker <ticker> --json` | **Fast single-token balance — use this for "how much of TOKEN do I have?" and pre-trade checks; prefer it over the full-portfolio forms below when you only need one token.** `--ticker` (alias: `--token`) is a ticker (e.g. `VIRTUAL`) or a contract/mint address. **`--chain-id` is optional and usually unnecessary** — omit it and the backend scans the wallet across every chain in ONE call and filters to the ticker, which is what you want when the user just names a ticker. Add `--chain-id <id>` (EVM id, or `500`/`501` for Solana) only when they named a chain; that mode instead resolves the ticker to its trade-canonical contract, which is what a pre-trade sizing check wants. A contract/mint address works with or without `--chain-id` — without one the scan matches the address on whichever chain it turns up, which is stricter than a ticker since it can't be confused by a same-ticker token. | Without `--chain-id`: `{symbol, total, balances:[{chainId, network, tokenAddress, symbol, walletAddress, raw, decimals, balance}], networksChecked:[…], unresolvedNetworks:[…], stocks:{positions:[…], unavailable}, hyperliquid:{spotBalances:[…], positions:[…]}\|null}`. `total` is the summed **on-chain** holding (already decimal-shifted; don't divide again) — tokenized stocks and Hyperliquid are reported in their own fields and deliberately NOT added into it, since shares and perp size aren't token balances. A network in `unresolvedNetworks`, or `stocks.unavailable:true`, means that part was not read — never report a zero `total` as "holds none" then. With `--chain-id`: the flat `{chainId, network, tokenAddress, symbol, walletAddress, raw, decimals, balance}` (`raw` = base units; `balance` = decimal-shifted string) |
| `acp wallet balance [--chain-id <id>] [--cluster <c>] --json` | Token balances (full portfolio; for a single token prefer `--ticker` above). No flags → all sponsored EVM chains + Solana for the env. `--chain-id` narrows to one chain (EVM, or `500`/`501` for Solana); `--cluster devnet\|mainnet` → Solana only | Single chain (`--chain-id`/`--cluster`): `{chainId, network, address, tokens:[…]}`. All-chains (no flags): `{chains:[{chainId, network}], address, solanaAddress, tokens:[…]}` — `tokens` each carry `network`; group by it. Each token: `{tokenAddress, tokenBalance, tokenMetadata:{symbol, name, decimals}, tokenPrices:[{value}]}` (`tokenBalance` raw; decimal-shift by `decimals`, native token has `tokenAddress:null`). The all-chains view also carries `stocks:[{ticker, token_ticker, chain, protocol, shares, usd_value, ...}]` — your tokenized-stock holdings resolved per venue (`chain` is the value a stock sell's `--chain` needs; `shares` is the sellable amount) — and a `hyperliquid` account summary |
...
# ...then sell with that chain (--chain eth|sol is REQUIRED on sells)
acp trade --token AAPL --amount-shares 0.01 --chain sol --json
# Tokenized stock: SELL 3 AAPL, receive proceeds as USDC on Base (eth venue bridges onward)
...
| `trade` (tokenized stock) | Spot buy/sell of a tokenized equity (you own the share token; NO `--side`). Backend auto-routes the venue/chain | `--token <TICKER>`, and `--amount-usdc` (buy) **or** `--amount-shares` + `--chain eth\|sol` (sell — get the chain from `stocks[]` in `acp wallet balance --json`) | buy can fund from another chain via `--token-in`/`--chain-in`/`--amount-in`; sell can redirect proceeds via `--token-out`/`--chain-out` (eth venue: USDC to any chain, or a non-USDC token when `--chain-out` is set); `--chain eth\|sol` (optional on buys to pin the venue), `--protocol`, `--slippage`, `--dry-run` |
