...
export function registerEmailCommands(program: Command): void {
  ...
  email
    .command("provision")
    .description(
      "Provision a new email identity for the active agent. The local part is auto-generated by the backend."
    )
    .action(async (_opts, cmd) => {
      const { agentApi } = await getClient();
      const json = isJson(cmd);
      ...
      try {
        const result = await agentApi.provisionEmailIdentity(agentId);
